...
%
% Unified paper combining:
%   weyl.tex                  — Complexified Weyl derivative and generalized zeta
%   schrodinger-riemann.tex   — Quantum-theoretic conjecture on zeros
...
%
% Status: informal / speculative notes unified for structural comparison.
% A single thread runs through all four: the zeros of zeta are spectral data of
% an operator, and the critical line emerges from a symmetry / reality condition.
...
Spectral Operators, Weyl Derivatives, Quantum Mechanics,\\
...
\begin{abstract}
We collect and compare four independent approaches toward understanding the
Riemann Hypothesis, noting that each encodes the same fundamental structure
from a different vantage point: \emph{the nontrivial zeros of $\zeta(s)$ are
spectral data of a naturally-arising operator, and the critical line $\Re(s)=\tfrac12$
emerges from a reality or symmetry condition on that operator}.

The first approach (Weyl derivative) decomposes $\partial^s f$ into a sum of
two generalized Dirichlet series and conjectures that generic disjointness of
their zero sets forces the critical-line symmetry condition $a_n = a_{-n}$ for
annihilation.  The second approach (Schr\"odinger--Riemann) identifies zeros
with forbidden normalization values of a quantum wave function.  The third
approach (Yang-Baxter / Virasoro) recognizes the decay exponent $\alpha$ as an
anomalous dimension playing the same role as the fine structure constant
$\alpha_{\mathrm{EM}}$ in QED and as $\alpha_\pm$ in the Coulomb-gas Virasoro
realization; the critical line follows from real grading of the Yang-Baxter
system.  The fourth approach (Volterra / operator algebras) reformulates the
problem as a vertical product integral over the critical strip, connects it to
von Neumann algebra traces and the Connes trace formula, and introduces a
Heisenberg algebra over primes in which integers are words and primes are
letters, with connections to the Langlands program.

None of these approaches is complete; they are collected here to expose the
common spectral thread and to make cross-pollination between them explicit.
\end{abstract}
...
\begin{tabular}{p{3.2cm} p{3.8cm} p{4.2cm}}
...
Schr\"odinger--Riemann (\S\ref{sec:schrodinger})
  & $-\tfrac12\partial_x^2$, Hamiltonian
...
Volterra / Langlands (\S\ref{sec:volterra})
  & Dirac $\mathcal{D}$ on adele class space; $H = \sum_p \log p\,a_p^\dagger a_p$
...
(with $a_0 = 0$) by multiplication of the modes by $(in)^s$:~\cite{weylref}
\[
\partial^s f(\theta) = \sum_{n=-\infty}^\infty (in)^s a_n e^{in\theta}.
\]
This operator is central to infinite-dimensional Lie theory: the Witt algebra
of derivations on the circle has $d_n = -e^{in\theta}\partial_\theta$ as
generators, and $\partial^s$ is the continuous interpolation of these modes.
...
\begin{conjecture}[Generic disjointness of zero sets~\cite{weylref}]
\label{conj:weyl}
...
\begin{conjecture}[Quantum-theoretic Riemann Hypothesis~\cite{schrodingerref}]
\label{conj:qzeta}
...
\[
\alpha_+\alpha_- = \tfrac14, \qquad c = 1 - 24\alpha_0^2, \qquad
h_{r,s} = \frac{(r\alpha_+ + s\alpha_-)^2 - Q^2/4}{1}.
\]
...
QED & $\alpha_{\mathrm{EM}}$ & $\approx 1/137$ &
  Anomalous dim.\ of photon; IR fixed point \\
...
Virasoro Coulomb gas & $\alpha_\pm$ & $\alpha_+\alpha_- = \tfrac14$ &
  Vertex-op.\ anomalous dim.; Kac table \\
\end{tabular}
\end{center}
...
\textbf{Approach} & \textbf{The symmetry / reality condition} \\
\hline
Weyl derivative & $a_n = a_{-n}$: real (cosine) Fourier series \\
Schr\"odinger & $A_n = A_{-n}$: spatially symmetric wave function \\
Yang-Baxter & $\alpha \in \R$: real decay exponent / real grading of the IBP system \\
Connes / Langlands & Partial trace $F(x)$ real-analytic off $x=1/2$; real spectrum of $L_0$ \\
\end{tabular}
\end{center}
...
\[
\partial^s f = i^s\,L(-s,\chi(n)) + (-i)^s\, L(-s,\chi(-n))
= \text{(left-moving contribution)} + \text{(right-moving contribution)},
\]
...
\hline
Weyl derivative & $\partial^s = i^s L(-s,\chi(n)) + \ldots$ &
  $Z_+ \cap Z_- = \emptyset$ $\Rightarrow$ $a_n = a_{-n}$ &
  Conjecture; not proved \\
Schr\"odinger & Forbidden normalization; $\|D(s)\|^2 = 0$ &
  $L^2$ symmetry; $A_n = A_{-n}$ &
  Conjecture; heuristic \\
Yang-Baxter & Real grading; quadratic factor $as^2{-}as{+}1$ &
  $\alpha \in \R$, $a < 4$ &
  \textbf{Complete}~\cite{companion} \\
Volterra / Langlands & Arg.\ principle; Connes trace; Fock space &
  Partial trace smooth off $x=\tfrac12$ &
  Conceptually correct; analytic gap \\
\end{tabular}
\end{center}
...
\bibitem{weylref}
D.~Remmenga,
\textit{A Note on the Complexified Weyl Derivative},
J.~Lie Theory (submitted), preprint (2026).

\bibitem{schrodingerref}
D.~Remmenga,
\textit{From Schr\"odinger to Riemann: A Quantum-Theoretic Conjecture},
preprint (2026).
