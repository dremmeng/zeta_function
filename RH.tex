...
\begin{proof}
By Theorem~\ref{thm:bell_polynomial},
$\star B_{n}(x)=\star(x)\,B_{n}(g'(x),\ldots,g^{(n)}(x))$.
The logarithmic derivatives satisfy $g^{(k)}(x)=2^{-k}\frac{d^{k-1}}{dx^{k-1}}\tanh(x/2)$;
for $k\ge 2$ these are polynomials in $\operatorname{sech}(x/2)$, so
$g^{(k)}(x)=O(e^{-x})\to 0$ as $x\to+\infty$.
Hence the Bell polynomial factor satisfies
$B_{n}(g'(x),\ldots,g^{(n)}(x))\to B_{n}(\tfrac{1}{2},0,\ldots,0)=2^{-n}$, which is
bounded.
From the zeta-function identity of Theorem~\ref{thm:sigma_zeta}
(cf.\ the relation $\sigma(s,0,0)=\int_{0}^{\infty}x^{s}e^{x}(e^{x}+1)^{-2}\,dx$), the
effective squared weight satisfies
$\star(x)^{2}=O\!\left(e^{x}(e^{x}+1)^{-2}\right)=O(e^{-x})$,
so $|\star(x)|=O(e^{-x/2})\to 0$ exponentially.
Combining these two estimates gives $|\star B_{n}(x)|=O(e^{-x/2})\to 0$.

\end{proof}
\begin{remark}
  The sense in which $\star B_n(\infty)$ does not vanish is addressed and renormalized in Section~\ref{sec:parabolic_coordinate} and onwards and is used to build the generating function $\mathcal{G}(z)$ for the zero set.
\end{remark}
